...
export default function Navbar() {
  ...
  return (
    <>
      <nav aria-label="Main navigation" className="fixed top-0 left-0 right-0 z-50 bg-white/95 backdrop-blur-xl shadow-sm border-b border-slate-100">
        <div className="max-w-6xl mx-auto px-6 h-16 flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center gap-7">
            {NAV_LINKS.map((link) =>
              link.dropdown ? (
                <DropdownItem key={link.label} link={link} scrolled={true} />
              ) : (
                <Link
                  key={link.label}
                  to={link.path}
            ...
            <Link
              to="/Apply"
              className="text-sm font-semibold px-5 py-2 rounded-full bg-teal-600 hover:bg-teal-700 text-white transition-all"
            >
              Join us!
            </Link>
          </div>
          ...
          {/* Mobile toggle */}
          <button
            className="md:hidden p-2 focus-visible:outline focus-visible:outline-2 focus-visible:outline-teal-500 rounded text-slate-700"
